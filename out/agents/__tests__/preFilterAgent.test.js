...
describe('PreFilterAgent', () => {
    ...
    beforeEach(() => {
        ...
        agent = new preFilterAgent_1.PreFilterAgent(mockOrchestrator);
    });
    describe('execute', () => {
        it('should optimize a simple prompt', async () => {
            ...
            expect(result.data.optimizedPrompt).toContain('fibonacci');
            expect(result.data.estimatedComplexity).toBe('moderate');
        ...
        it('should handle complex prompts with codebase context', async () => {
            ...
            const result = await agent.execute(task);
            expect(result.success).toBe(true);
            expect(result.data.estimatedComplexity).toBe('complex');
            expect(result.data.contextAdded.length).toBeGreaterThan(0);
        });
    ...
    describe('estimateComplexity', () => {
        ...
        it('should return moderate for medium prompts with context', () => {
            const complexity = agent.estimateComplexity('Create a new component that displays user information', { codebaseContext: 'some context' });
...
//# sourceMappingURL=preFilterAgent.test.js.map