...
describe('GitService', () => {
    ...
    beforeEach(() => {
        gitService = new gitService_1.GitService(mockWorkspaceRoot);
        jest.clearAllMocks();
    });
    afterEach(() => {
        gitService.dispose();
    ...
    describe('parseFileStatus', () => {
        it('should parse file status characters correctly', () => {
            const parseFileStatus = gitService.parseFileStatus.bind(gitService);
            expect(parseFileStatus('A')).toBe(gitService_1.GitFileStatus.ADDED);
            expect(parseFileStatus('M')).toBe(gitService_1.GitFileStatus.MODIFIED);
            expect(parseFileStatus('D')).toBe(gitService_1.GitFileStatus.DELETED);
            expect(parseFileStatus('R')).toBe(gitService_1.GitFileStatus.RENAMED);
            expect(parseFileStatus('C')).toBe(gitService_1.GitFileStatus.COPIED);
            expect(parseFileStatus('?')).toBe(gitService_1.GitFileStatus.UNTRACKED);
            expect(parseFileStatus('U')).toBe(gitService_1.GitFileStatus.UNMERGED);
        });
    });
});
