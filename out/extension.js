...
const analyticsViewProvider_1 = require("./analytics/analyticsViewProvider");
const inlineCompletionProvider_1 = require("./completion/inlineCompletionProvider");
const contextPanel_1 = require("./ui/contextPanel");
...
async function activate(context) {
    ...
    try {
        ...
        const storageManager = new secureStorage_1.SecureStorageManager(context);
        const statusBar = new statusBar_1.StatusBarManager();
        // Initialize extension manager
        extensionManager = new extensionManager_1.ExtensionManager(context, configManager, storageManager, statusBar);
        ...
        const chatProvider = new chatViewProvider_1.ChatViewProvider(context.extensionUri, extensionManager);
        ...
        const contextPanel = new contextPanel_1.ContextPanelProvider(context.extensionUri);
        context.subscriptions.push(vscode.window.registerWebviewViewProvider(contextPanel_1.ContextPanelProvider.viewType, contextPanel));
        ...
        if (analyticsManager) {
            const analyticsProvider = new analyticsViewProvider_1.AnalyticsViewProvider(context.extensionUri, analyticsManager);
            context.subscriptions.push(vscode.window.registerWebviewViewProvider(analyticsViewProvider_1.AnalyticsViewProvider.viewType, analyticsProvider));
        }
        // Register inline completion provider
        const orchestrator = extensionManager.getModelOrchestrator();
        const ragService = extensionManager.getRAGService() || null;
        if (orchestrator) {
            const inlineCompletionProvider = new inlineCompletionProvider_1.InlineCompletionProvider(orchestrator, ragService);
            ...
            for (const language of supportedLanguages) {
                context.subscriptions.push(vscode.languages.registerInlineCompletionItemProvider({ language }, inlineCompletionProvider));
            }
        ...
        vscode.window.showErrorMessage(`Failed to activate Codelicious: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
/**
 ...
 */
function registerCommands(context, manager) {
    // Open chat command
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.openChat', async () => {
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.configureApiKeys', async () => {
        await manager.configureApiKeys();
    }));
    // Reindex project command
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.reindexProject', async () => {
        await manager.reindexProject();
    }));
    // Show index status command
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.showIndexStatus', async () => {
        await manager.showIndexStatus();
    }));
    // Clear cache command
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.clearCache', async () => {
        await manager.clearCache();
    }));
    // Show cost tracking command
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.showCostTracking', async () => {
        await manager.showCostTracking();
    }));
    // Show analytics dashboard command
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.showAnalytics', async () => {
        await vscode.commands.executeCommand('codelicious.analyticsView.focus');
    }));
    // Multi-file edit command
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.multiFileEdit', async () => {
    ...
    // Code review command
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.reviewCode', async () => {
        ...
        try {
            ...
            if (criticalCount > 0 || highCount > 0) {
                vscode.window.showWarningMessage(`Code review found ${criticalCount} critical and ${highCount} high priority issues. Score: ${result.score}/100`);
            }
            else {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.generateDocs', async () => {
        const generator = manager.getDocumentationGenerator();
        if (!generator) {
            vscode.window.showErrorMessage('Documentation generator not available');
            return;
        }
        const docType = await vscode.window.showQuickPick(['README', 'API Reference', 'Usage Guide', 'Architecture'], { placeHolder: 'Select documentation type' });
        if (!docType) {
            return;
        }
        try {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.selectModel', async () => {
        const orchestrator = manager.getModelOrchestrator();
        if (orchestrator) {
            const selector = new modelSelector_1.ModelSelector(orchestrator, context.extensionUri);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.configureAgentModels', async () => {
        await configureAgentModels();
    }));
    // Build persistent index command
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.buildPersistentIndex', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.queryArchitecture', async () => {
        const ragService = manager.getRAGService();
        if (!ragService) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.showIndexStats', async () => {
        const ragService = manager.getRAGService();
        ...
        vscode.window.showInformationMessage(`Index Stats - Queue: ${stats.queueSize}, Processing: ${stats.isProcessing ? 'Yes' : 'No'}`);
    }));
    // Autonomous build command
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.autonomousBuild', async () => {
        ...
        try {
            ...
            if (result.success) {
                vscode.window.showInformationMessage(` Build complete! Generated ${result.filesWritten.length} files in ${(result.duration / 1000).toFixed(1)}s. ` +
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.buildFromInstructionFile', async (uri) => {
        ...
        await vscode.window.withProgress({
            ...
            try {
                ...
                if (result.success) {
                    vscode.window.showInformationMessage(` Product built successfully! Generated ${result.filesGenerated} files with quality score ${result.qualityScore}/100`);
                }
                else {
                    vscode.window.showErrorMessage(` Build failed: ${result.errors?.join(', ')}`);
                }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.buildFromWorkspaceInstructions', async () => {
        const productBuilder = manager.getProductBuilder();
        if (!productBuilder) {
            vscode.window.showErrorMessage('Product builder not available');
            return;
        ...
        await vscode.window.withProgress({
            ...
            try {
                ...
                if (successCount > 0) {
                    vscode.window.showInformationMessage(` Built ${successCount} product(s) successfully! Generated ${totalFiles} files total`);
                }
                else {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.analyzeTests', async () => {
...
${result.summary.priorities.map((p, i) => `${i + 1}. ${p}`).join('\n')}`;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.fixFailingTests', async () => {
        const advancedTesting = manager.getAdvancedTesting();
        if (!advancedTesting) {
            vscode.window.showErrorMessage('Advanced testing not available');
            return;
        ...
        try {
            ...
            vscode.window.showInformationMessage(` Fixed ${result.successful}/${result.applied} tests in ${(result.duration / 1000).toFixed(1)}s`);
        }
        catch (error) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.compareModels', async () => {
        const orchestrator = manager.getModelOrchestrator();
        if (orchestrator) {
            const selector = new modelSelector_1.ModelSelector(orchestrator, context.extensionUri);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.generateTests', async () => {
    ...
    // Run tests command
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.runTests', async () => {
        const testManager = manager.getTestManager();
        if (testManager) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.runTestsForFile', async () => {
        const testManager = manager.getTestManager();
        if (testManager) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.runTestsWithCoverage', async () => {
        const testManager = manager.getTestManager();
        if (testManager) {
    ...
    // Stop tests command
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.stopTests', () => {
        const testManager = manager.getTestManager();
        if (testManager) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.analyzeFile', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.analyzeWorkspace', async () => {
        const intelligenceManager = manager.getIntelligenceManager();
        if (intelligenceManager) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.analyzeDependencies', async () => {
        const intelligenceManager = manager.getIntelligenceManager();
        if (intelligenceManager) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.extractMethod', async () => {
        const intelligenceManager = manager.getIntelligenceManager();
        if (intelligenceManager) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.extractVariable', async () => {
        const intelligenceManager = manager.getIntelligenceManager();
        if (intelligenceManager) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.showCodeQualityReport', async () => {
        const intelligenceManager = manager.getIntelligenceManager();
        if (intelligenceManager) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.processSpecification', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.processSpecificationFile', async (uri) => {
        const specManager = manager.getSpecificationManager();
        if (!specManager) {
            vscode.window.showErrorMessage('Specification manager not initialized');
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.parseSpecification', async () => {
        const specManager = manager.getSpecificationManager();
        if (!specManager) {
            vscode.window.showErrorMessage('Specification manager not initialized');
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.showSpecificationStatus', () => {
        const specManager = manager.getSpecificationManager();
        if (!specManager) {
            vscode.window.showErrorMessage('Specification manager not initialized');
            return;
        ...
        vscode.window.showInformationMessage(`Specification Status: ${status.state} (${status.progress}%)`);
    }));
    // Generate commit message command
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.generateCommitMessage', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.generatePRDescription', async () => {
        const gitManager = manager.getGitManager();
        if (!gitManager) {
            vscode.window.showErrorMessage('Git manager not initialized');
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.analyzeChanges', async () => {
        const gitManager = manager.getGitManager();
        if (!gitManager) {
            vscode.window.showErrorMessage('Git manager not initialized');
            return;
 ...
 */
async function configureAgentModels() {
 ...
 */
function registerPatternCommands(context, manager) {
    // View learned patterns command
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.viewLearnedPatterns', async () => {
        const learningManager = manager.getLearningManager();
        if (!learningManager) {
            vscode.window.showErrorMessage('Learning manager not initialized');
            return;
        }
        const patternLearner = learningManager.patternLearner;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.optimizePatternCache', async () => {
        const learningManager = manager.getLearningManager();
        if (!learningManager) {
            vscode.window.showErrorMessage('Learning manager not initialized');
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.exportPatterns', async () => {
        const learningManager = manager.getLearningManager();
        if (!learningManager) {
            vscode.window.showErrorMessage('Learning manager not initialized');
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.triggerLearning', async () => {
        const learningManager = manager.getLearningManager();
        if (!learningManager) {
            vscode.window.showErrorMessage('Learning manager not initialized');
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.showCacheStats', async () => {
        const learningManager = manager.getLearningManager();
        if (!learningManager) {
            vscode.window.showErrorMessage('Learning manager not initialized');
            return;
        }
        const stats = learningManager.getCacheStats(); // Complex cache statistics structure
        if (!stats.available) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.showQuickActions', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.startTutorial', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.showHealthStatus', async () => {
        const orchestrator = manager.getModelOrchestrator();
        if (!orchestrator) {
            vscode.window.showErrorMessage('Model orchestrator not initialized');
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.showPerformanceMetrics', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.showActivityLog', async () => {
        const orchestrator = manager.getModelOrchestrator();
        if (!orchestrator) {
            vscode.window.showErrorMessage('Model orchestrator not initialized');
            return;
        ...
        const queueData = Object.values(queueStats)[0] || {
            queueSize: 0,
            activeRequests: 0,
            totalProcessed: 0,
            totalFailed: 0
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.runConfigurationWizard', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.listMCPTools', async () => {
        ...
        if (selected && 'tool' in selected) {
            ...
            const message = [
                ...
                '',
                `**Capabilities:**`,
                ...tool.capabilities.map((cap) => ` • ${cap}`),
                '',
                `**Parameters:** ${tool.parameters.length}`,
                ...tool.parameters.map((p) => ` • ${p.name} (${p.type})${p.required ? ' *required*' : ''}: ${p.description}`)
            ].join('\n');
            const doc = await vscode.workspace.openTextDocument({
                content: message,
                language: 'markdown'
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.showMCPStats', async () => {
        const mcpRegistry = manager.getMCPRegistry();
        if (!mcpRegistry) {
            vscode.window.showErrorMessage('MCP Tool Registry not available');
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codelicious.searchMCPTools', async () => {
        const mcpRegistry = manager.getMCPRegistry();
        if (!mcpRegistry) {
            vscode.window.showErrorMessage('MCP Tool Registry not available');
            return;
