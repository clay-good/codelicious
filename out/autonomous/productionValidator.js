...
const logger = (0, logger_1.createLogger)('ProductionValidator');
class ProductionValidator {
    constructor(executionEngine, workspaceRoot) {
    ...
    */
    async validate(generatedCode, generatedTests, requirements) {
    ...
    */
    async checkCompilation(generatedCode) {
        const issues = [];
        const warnings = [];
        const suggestions = [];
        try {
            // Try to compile TypeScript
            const result = await this.executionEngine.execute('npx tsc --noEmit', {
    ...
    * Check if tests pass
    */
    async checkTests(generatedTests) {
        const issues = [];
        const warnings = [];
        const suggestions = [];
        ...
        try {
            // Run tests
            const result = await this.executionEngine.execute('npm test', {
                workingDirectory: this.workspaceRoot,
                timeout: 60000,
                requireConfirmation: false
            });
    ...
    * Check linting
    */
    async checkLinting(generatedCode) {
        const issues = [];
        const warnings = [];
        const suggestions = [];
        try {
            // Run ESLint
            const result = await this.executionEngine.execute('npx eslint . --ext .ts,.tsx,.js,.jsx', {
                workingDirectory: this.workspaceRoot,
                timeout: 30000,
                requireConfirmation: false
            });
    ...
    * Check security
    */
    async checkSecurity(generatedCode) {
        const issues = [];
        const warnings = [];
        const suggestions = [];
    ...
    */
    async checkRequirements(generatedCode, requirements) {
        const issues = [];
        const warnings = [];
        const suggestions = [];
    ...
    * Check performance
    */
    async checkPerformance(generatedCode) {
        const issues = [];
        const warnings = [];
        const suggestions = [];
    ...
    * Check documentation
    */
    async checkDocumentation(generatedCode) {
        const issues = [];
        const warnings = [];
        const suggestions = [];
        let filesWithDocs = 0;
    ...
    * Generate summary
    */
    generateSummary(checks, score, passed) {
...
//# sourceMappingURL=productionValidator.js.map