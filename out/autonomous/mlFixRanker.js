...
exports.MLFixRanker = void 0;
const logger_1 = require("../utils/logger");
const logger = (0, logger_1.createLogger)('MLFixRanker');
class MLFixRanker {
    ...
    */
    async rankFixes(fixes, error, context) {
        const rankedFixes = [];
    ...
    */
    async extractFeatures(fix, error, context) {
        // Historical features
    ...
    */
    calibrateConfidence(originalConfidence, features) {
    ...
    */
    getHistoricalFixes(error, fix) {
        return this.fixHistory.filter(h => h.errorType === error.type &&
    ...
    */
    async calculateCodeSimilarity(fix, fileContent) {
        try {
            // Get embeddings for fix code and file content
            const fixCode = fix.changes.map((c) => c.newCode).join('\n'); // Fix change structure
            ...
            const fileEmbedding = await this.embeddingManager.generateEmbedding(fileContent.substring(0, contextWindow));
    ...
    */
    calculateErrorSimilarity(error, historicalFixes) {
    ...
    */
    async calculateContextSimilarity(fix, fileContent) {
        try {
    ...
    */
    extractFileContext(fileContent) {
    ...
    */
    recordFixOutcome(fix, error, features, predictedScore, actualSuccess) {
        // Add to fix history
    ...
    */
    updateMetrics() {
        if (this.rankingHistory.length === 0)
            return;
    ...
    */
    calculateFeatureImportance() {
        ...
        for (const feature of features) {
            ...
            let count = 0;
            for (const hist of this.rankingHistory) {
                const featureValue = hist.features[feature] || 0;
...
exports.MLFixRanker = MLFixRanker;
//# sourceMappingURL=mlFixRanker.js.map