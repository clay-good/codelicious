...
describe('SpecificationParser', () => {
    ...
    beforeEach(() => {
        parser = new specificationParser_1.SpecificationParser(mockWorkspaceRoot);
    });
    describe('parse', () => {
...
4. Users can view their profile (functional)
 `;
            const result = await parser.parse(text);
            expect(result.requirements[0].type).toBe(specificationParser_1.RequirementType.PERFORMANCE);
            expect(result.requirements[1].type).toBe(specificationParser_1.RequirementType.SECURITY);
            expect(result.requirements[2].type).toBe(specificationParser_1.RequirementType.TECHNICAL);
            expect(result.requirements[3].type).toBe(specificationParser_1.RequirementType.FUNCTIONAL);
        });
...
4. Normal requirement
 `;
            const result = await parser.parse(text);
            expect(result.requirements[0].priority).toBe(specificationParser_1.Priority.CRITICAL);
            expect(result.requirements[1].priority).toBe(specificationParser_1.Priority.HIGH);
            expect(result.requirements[2].priority).toBe(specificationParser_1.Priority.LOW);
            expect(result.requirements[3].priority).toBe(specificationParser_1.Priority.MEDIUM);
        });
    });
...
6. Document the API
 `;
            const result = await parser.parse(text);
            expect(result.tasks[0].type).toBe(specificationParser_1.TaskType.CREATE);
            expect(result.tasks[1].type).toBe(specificationParser_1.TaskType.MODIFY);
            expect(result.tasks[2].type).toBe(specificationParser_1.TaskType.DELETE);
            expect(result.tasks[3].type).toBe(specificationParser_1.TaskType.REFACTOR);
            expect(result.tasks[4].type).toBe(specificationParser_1.TaskType.TEST);
            expect(result.tasks[5].type).toBe(specificationParser_1.TaskType.DOCUMENT);
        });
...
5. Must be fast (performance)
 `;
            const result = await parser.parse(text);
            expect(result.constraints[0].type).toBe(specificationParser_1.ConstraintType.TIME);
            expect(result.constraints[1].type).toBe(specificationParser_1.ConstraintType.BUDGET);
            expect(result.constraints[2].type).toBe(specificationParser_1.ConstraintType.TECHNOLOGY);
            expect(result.constraints[3].type).toBe(specificationParser_1.ConstraintType.SECURITY);
            expect(result.constraints[4].type).toBe(specificationParser_1.ConstraintType.PERFORMANCE);
        });
    });
...
//# sourceMappingURL=specificationParser.test.js.map