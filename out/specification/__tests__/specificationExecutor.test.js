...
describe('SpecificationExecutor', () => {
    ...
    beforeEach(() => {
        ...
        executor = new specificationExecutor_1.SpecificationExecutor(mockWorkspaceRoot, mockOutputChannel);
    });
    const createMockPlan = (tasks) => {
        const plannedTasks = tasks.map((t, i) => ({
            id: t.id || `task-${i + 1}`,
            name: t.name || `Task ${i + 1}`,
            description: t.description || `Description ${i + 1}`,
            type: t.type || specificationParser_1.TaskType.CREATE,
            priority: t.priority || specificationParser_1.Priority.MEDIUM,
    ...
    describe('execute', () => {
        it('should execute simple plan successfully', async () => {
            const plan = createMockPlan([
                { name: 'Task 1', type: specificationParser_1.TaskType.CREATE, files: [] }
            ]);
        ...
        it('should execute multiple tasks', async () => {
            const plan = createMockPlan([
                { name: 'Task 1', type: specificationParser_1.TaskType.CREATE, files: [] },
                { name: 'Task 2', type: specificationParser_1.TaskType.CREATE, files: [] },
                { name: 'Task 3', type: specificationParser_1.TaskType.CREATE, files: [] }
            ]);
            const result = await executor.execute(plan, { dryRun: true });
            expect(result.success).toBe(true);
        ...
        it('should handle dry run mode', async () => {
            const plan = createMockPlan([
                { name: 'Task 1', type: specificationParser_1.TaskType.CREATE, files: ['test.ts'] }
            ]);
            const result = await executor.execute(plan, { dryRun: true });
            expect(result.success).toBe(true);
            expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(expect.stringContaining('Dry Run'));
        });
    ...
    describe('execution options', () => {
        it('should stop on error when stopOnError is true', async () => {
            ...
            jest.spyOn(executor, 'executeTask').mockRejectedValueOnce(new Error('Task failed'));
    ...
    describe('task execution', () => {
        ...
        it('should handle MODIFY tasks', async () => {
            const plan = createMockPlan([
                {
                    name: 'Modify file',
                    type: specificationParser_1.TaskType.MODIFY,
                    files: []
                }
            ]);
            const result = await executor.execute(plan, { dryRun: true });
        ...
        it('should handle DELETE tasks', async () => {
            const plan = createMockPlan([
                {
                    name: 'Delete file',
                    type: specificationParser_1.TaskType.DELETE,
                    files: []
                }
            ]);
            const result = await executor.execute(plan, { dryRun: true });
        ...
        it('should handle REFACTOR tasks', async () => {
            const plan = createMockPlan([
                {
                    name: 'Refactor code',
                    type: specificationParser_1.TaskType.REFACTOR,
                    files: []
                }
            ]);
            const result = await executor.execute(plan, { dryRun: true });
        ...
        it('should handle TEST tasks', async () => {
            const plan = createMockPlan([
                {
                    name: 'Run tests',
                    type: specificationParser_1.TaskType.TEST,
                    tests: []
                }
            ]);
            const result = await executor.execute(plan, { dryRun: true });
        ...
        it('should handle DOCUMENT tasks', async () => {
            const plan = createMockPlan([
                {
                    name: 'Generate docs',
                    type: specificationParser_1.TaskType.DOCUMENT,
                    files: []
                }
            ]);
            const result = await executor.execute(plan, { dryRun: true });
    ...
    describe('error handling', () => {
        it('should capture task errors', async () => {
            ...
            jest.spyOn(executor, 'executeTask').mockRejectedValueOnce(new Error('Test error'));
    ...
    describe('output logging', () => {
        it('should log execution start', async () => {
            ...
            expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(expect.stringContaining('Starting Specification Execution'));
        });
        it('should log execution complete', async () => {
            ...
            await executor.execute(plan, { dryRun: true });
            expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(expect.stringContaining('Execution Complete'));
        });
        it('should log task execution', async () => {
            const plan = createMockPlan([
                { name: 'My Task', type: specificationParser_1.TaskType.CREATE, files: [] }
            ]);
            await executor.execute(plan, { dryRun: true });
            expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(expect.stringContaining('My Task'));
        });
    });
    describe('file generation templates', () => {
        it('should generate TypeScript class template', () => {
            const task = {
                ...
                type: specificationParser_1.TaskType.CREATE,
                priority: specificationParser_1.Priority.HIGH,
                estimatedTime: 60,
                dependencies: [],
                requirements: [],
            ...
            const content = executor.generateFileContent('userService.ts', task);
        ...
        it('should generate JavaScript class template', () => {
            ...
            const content = executor.generateFileContent('dataHandler.js', task);
        ...
        it('should generate Python class template', () => {
            ...
            const content = executor.generateFileContent('api_client.py', task);
        ...
        it('should generate test template for TypeScript', () => {
            ...
            const content = executor.generateFileContent('userService.test.ts', task);
        ...
        it('should generate Markdown template', () => {
            ...
            const content = executor.generateFileContent('API.md', task);
        ...
        it('should convert file names to PascalCase', () => {
            const toPascalCase = executor.toPascalCase.bind(executor);
...
//# sourceMappingURL=specificationExecutor.test.js.map