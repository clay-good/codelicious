...
const autonomousBuilder_1 = require("../autonomous/autonomousBuilder");
...
const chatViewTemplate_1 = require("./chatViewTemplate");
const chatMessageHandler_1 = require("./chatMessageHandler");
const logger_1 = require("../utils/logger");
const logger = (0, logger_1.createLogger)('ChatViewProvider');
class ChatViewProvider {
    constructor(_extensionUri, extensionManager) {
        this._extensionUri = _extensionUri;
        this.extensionManager = extensionManager;
        this.conversationHistory = [];
        this.lastAIResponse = '';
        this.sessionInitialized = false;
        this.multiAgentEnabled = true; // Enable multi-agent by default
        this.autoBuildEnabled = true; // Enable automatic build detection by default
    }
    /**
    ...
    */
    ...
    resolveWebviewView(webviewView, context, _token) {
        ...
        if (orchestrator) {
            this.modelSelector = new modelSelector_1.ModelSelector(orchestrator);
            this.selectedModel = this.modelSelector.getDefaultModel();
            // Initialize intelligent request router
            this.requestRouter = new intelligentRequestRouter_1.IntelligentRequestRouter(orchestrator);
            // Initialize message handler
            this.messageHandler = new chatMessageHandler_1.ChatMessageHandler({
                orchestrator,
        ...
        if (workspaceFolders && workspaceFolders.length > 0) {
            ...
            this.codeActionHandler = new codeActionHandler_1.CodeActionHandler(workspaceRoot);
            this.fileAttachmentManager = new fileAttachmentManager_1.FileAttachmentManager(workspaceRoot);
            this.autonomousExecutor = new autonomousExecutor_1.AutonomousExecutor(workspaceRoot);
            ...
            if (orchestrator) {
                ...
                if (executionEngine) {
                    this.agentOrchestrator = new agentOrchestrator_1.AgentOrchestrator(orchestrator, executionEngine, workspaceRoot);
                }
            }
        }
    ...
    async handleUserMessage(message) {
        ...
        if (this.requestRouter && this.autoBuildEnabled) {
            try {
                ...
                const shouldBuild = intent.type === intelligentRequestRouter_1.RequestType.BUILD_REQUEST ||
                    intent.type === intelligentRequestRouter_1.RequestType.CODE_QUESTION ||
                    intent.type === intelligentRequestRouter_1.RequestType.CODE_EXPLANATION ||
                    intent.type === intelligentRequestRouter_1.RequestType.CODE_REVIEW ||
                    intent.type === intelligentRequestRouter_1.RequestType.DEBUGGING;
                if (shouldBuild) {
        ...
        try {
            ...
            const enableStreaming = config.get('chat.enableStreaming', true);
            if (enableStreaming) {
    ...
    */
    async handleStreamingResponse(orchestrator, // ModelOrchestrator - using any due to complex generic types
    ...
    */
    async handleNonStreamingResponse(orchestrator, // ModelOrchestrator - using any due to complex generic types
    messages, userMessage) {
        if (!this._view) {
            return;
        }
    ...
    */
    updateModelInfo() {
        ...
        const modelInfo = modelSelector_1.ModelSelector.getModelInfo(this.selectedModel);
        if (modelInfo) {
            this._view.webview.postMessage({
                type: 'updateModel',
                model: {
 ...
 */
 ...
 messageInput.focus();
    ...
    */
    async handleApplyCode(code, language) {
    ...
    */
    async handleExplainCode(code, language) {
        if (!this.codeActionHandler) {
            vscode.window.showErrorMessage('Code action handler not initialized');
            return;
        }
    ...
    */
    async handleRunCode(code, language) {
        if (!this.codeActionHandler) {
            vscode.window.showErrorMessage('Code action handler not initialized');
            return;
        }
    ...
    */
    async handleRemoveAttachment(filePath) {
        if (!this.fileAttachmentManager || !this._view) {
            return;
        }
    ...
    */
    async checkForAutonomousOperations(aiResponse) {
    ...
    */
    async handleExecuteAutonomous() {
        ...
        try {
            ...
            // Show result
            if (result.success) {
                vscode.window.showInformationMessage(` Successfully applied ${result.appliedOperations.length} operation(s)!`);
                // Notify webview
                this._view.webview.postMessage({
                ...
                vscode.window.showWarningMessage(` Applied ${result.appliedOperations.length} operation(s), ${result.failedOperations.length} failed`);
                // Show errors
            ...
            vscode.window.showErrorMessage(`Failed to execute autonomous operations: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
    ...
    */
    getBuilderModeMessage(intent) {
        ...
        switch (intent.type) {
            ...
            case intelligentRequestRouter_1.RequestType.CODE_QUESTION:
                return ` **BUILDER MODE: Example + Explanation**\n\n` +
                    `I'll create working example files to answer your question:\n${baseInfo}\n\n` +
                    `Creating example files... `;
            case intelligentRequestRouter_1.RequestType.CODE_EXPLANATION:
    ...
    */
    isAutonomousBuildRequest(message) {
    ...
    */
    async handleAutonomousBuildRequest(message) {
        if (!this._view) {
            return;
        }
        // Show user message
        ...
        if (this.fileAttachmentManager && this.fileAttachmentManager.getCount() > 0) {
            ...
            const specFile = attachments.find((a) => a.path.endsWith('.txt') || a.path.endsWith('.md'));
            if (specFile) {
        ...
        const proceed = await vscode.window.showInformationMessage(` Start Autonomous Build?\n\nProject: ${projectName}\n\nThis will run autonomously until the project is complete. You can cancel at any time.`, { modal: true }, 'Start Build', 'Cancel');
    ...
    */
    async handleStartAutonomousBuild(specification, projectName) {
        if (!this._view) {
            return;
        }
        const workspaceFolders = vscode.workspace.workspaceFolders;
        ...
        this.autonomousBuilder = new autonomousBuilder_1.AutonomousBuilder(workspaceRoot, orchestrator, this.autonomousExecutor, executionEngine, ragService, learningManager, {
            maxIterations: 100,
    ...
    */
    async handleCancelAutonomousBuild() {
    ...
    */
    async handleMultiAgentCodeGeneration(message) {
        ...
        try {
            ...
            const autoWriteFiles = config.get('autoWriteFiles', true);
            const autoExecuteTests = config.get('autoExecuteTests', true);
            const requireApproval = config.get('requireApproval', true);
            // Execute multi-agent workflow
            const result = await this.agentOrchestrator.executeCodeGenerationWorkflow(message, this.conversationHistory, {
                codebaseContext,
                currentFile,
                autoWriteFiles,
                autoExecuteTests,
            ...
            if (result.success) {
                ...
                const finalOutput = result.finalOutput; // Complex workflow output structure
                // Add generated code
                if (finalOutput.code) {
                ...
                const securityResult = result.agentResults.get('security_reviewer');
                if (securityResult && securityResult.data && typeof securityResult.data === 'object') {
                    const secData = securityResult.data;
    ...
    */
    async trackMessageInSession(message) {
    ...
    */
    extractContextSources(ragResponse) {
        const sources = [];
...
ChatViewProvider.viewType = 'codelicious.chatView';
