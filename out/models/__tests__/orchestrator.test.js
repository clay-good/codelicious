...
describe('ModelOrchestrator', () => {
    ...
    beforeEach(() => {
        ...
        mockConfigManager = {
            getModelsConfig: jest.fn().mockReturnValue({
                ...
                costLimit: 10.0,
                defaultProvider: types_1.ModelProvider.CLAUDE
            })
        };
        mockStorageManager = {
        ...
        orchestrator = new orchestrator_1.ModelOrchestrator(mockContext, mockConfigManager, mockStorageManager, mockCacheManager);
    });
    ...
    describe('cost management', () => {
        ...
        it('should throw error when no providers available', async () => {
            ...
            await expect(orchestrator.sendRequest(request)).rejects.toThrow('No AI providers available');
        });
    });
    describe('error handling', () => {
        it('should throw error if not initialized', async () => {
            ...
            await expect(orchestrator.sendRequest(request)).rejects.toThrow('ModelOrchestrator not initialized');
        });
        it('should throw error for streaming if not initialized', async () => {
            const request = {
                messages: [{ role: 'user', content: 'Hello' }]
            };
            await expect(orchestrator.sendStreamingRequest(request, () => { })).rejects.toThrow('ModelOrchestrator not initialized');
        });
    });
    describe('caching', () => {
    ...
    describe('sendRequest with providers', () => {
        beforeEach(async () => {
            ...
            mockStorageManager.get = jest.fn().mockImplementation((key) => {
    ...
    describe('budget enforcement', () => {
        it('should throw error when budget limit exceeded', async () => {
            ...
            await expect(orchestrator.sendRequest(request)).rejects.toThrow(/No AI providers available|Budget limit exceeded/);
        });
    ...
    describe('streaming requests', () => {
        it('should throw error for streaming if not initialized', async () => {
            ...
            await expect(orchestrator.sendStreamingRequest(request, onChunk)).rejects.toThrow('ModelOrchestrator not initialized');
        });
        it('should throw error for streaming with no providers', async () => {
            ...
            const onChunk = jest.fn();
            await expect(orchestrator.sendStreamingRequest(request, onChunk)).rejects.toThrow('No AI providers available');
        });
        it('should check budget before streaming', async () => {
            ...
            await expect(orchestrator.sendStreamingRequest(request, onChunk)).rejects.toThrow(/No AI providers available|Budget limit exceeded/);
        });
    });
    describe('provider adapter management', () => {
        it('should get adapter for specific provider', async () => {
            await orchestrator.initialize();
            const adapter = orchestrator.getAdapter(types_1.ModelProvider.CLAUDE);
