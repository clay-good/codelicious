...
const enhancedExecutionEngine_1 = require("../enhancedExecutionEngine");
...
describe('EnhancedExecutionEngine', () => {
    ...
    beforeEach(() => {
        ...
        engine = new enhancedExecutionEngine_1.EnhancedExecutionEngine(configManager);
    });
    afterEach(async () => {
    ...
    describe('Retry Logic', () => {
        ...
        it.skip('should retry failed commands', async () => {
            ...
            const result = await engine.executeWithRetry('node --invalid-flag', // Invalid flag causes failure
            {}, {
                maxRetries: 2,
                initialBackoffMs: 50,
                onRetry: (attempt) => {
        ...
        it.skip('should succeed on retry', async () => {
            ...
            let retryCount = 0;
            const result = await engine.executeWithRetry('node --version', {}, {
                maxRetries: 3,
                onRetry: () => retryCount++
            });
        ...
        it.skip('should use exponential backoff', async () => {
            ...
            let retryCount = 0;
            await engine.executeWithRetry('node --invalid-flag', {}, {
                maxRetries: 3,
                initialBackoffMs: 50,
                backoffMultiplier: 2,
                onRetry: () => {
    ...
    describe('Profiles', () => {
        ...
        it('should execute with profile', async () => {
            const result = await engine.executeWithProfile('echo "test"', 'development');
            expect(result.success).toBe(true);
        });
        it('should throw error for unknown profile', async () => {
            await expect(engine.executeWithProfile('echo "test"', 'unknown')).rejects.toThrow('Profile not found');
        });
    });
    // ==========================================================================
    // Template Tests
    ...
    describe('Templates', () => {
        ...
        it('should execute template with variables', async () => {
            const result = await engine.executeTemplate('build', {}, { timeout: 10000 });
            // Build may fail if no package.json, but template execution should work
            expect(result).toBeDefined();
        });
        it('should throw error for unknown template', async () => {
            await expect(engine.executeTemplate('unknown', {})).rejects.toThrow('Template not found');
        });
    });
    // ==========================================================================
    // Metrics Tests
...
//# sourceMappingURL=enhancedExecutionEngine.test.js.map