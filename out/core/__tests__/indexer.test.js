...
describe('IndexingEngine', () => {
    ...
    beforeEach(() => {
        ...
        indexingEngine = new indexer_1.IndexingEngine(mockContext, configManager, statusBar);
    });
    afterEach(async () => {
    ...
    describe('initialization', () => {
        it('should initialize with correct default state', () => {
            ...
            expect(status.phase).toBe(types_1.IndexingPhase.BASIC);
    ...
    describe('file operations', () => {
        it('should handle indexFile without errors', async () => {
            await expect(indexingEngine.indexFile('/fake/file.ts')).resolves.not.toThrow();
        });
        it('should handle updateFile without errors', async () => {
            await expect(indexingEngine.updateFile('/fake/file.ts')).resolves.not.toThrow();
        });
        it('should handle removeFile without errors', async () => {
            await expect(indexingEngine.removeFile('/fake/file.ts')).resolves.not.toThrow();
        });
    });
    ...
    describe('lifecycle', () => {
        ...
        it('should handle reindex request', async () => {
            await expect(indexingEngine.reindex()).resolves.not.toThrow();
        });
    });
