...
const logger = (0, logger_1.createLogger)('Retriever');
class RAGRetriever {
    constructor(embeddingManager, vectorStore, indexingEngine) {
    ...
    */
    calculateKeywordScore(keywords, file) {
        let score = 0;
    ...
    */
    getFileContent(file) {
        try {
            ...
            const content = (0, fileUtils_1.readFileContent)(file.path);
            if (content) {
                return content;
            }
        }
    ...
    */
    applyMMR(results, limit, lambda = 0.7) {
    ...
    */
    addContext(results, contextLines) {
        return results.map(result => {
            ...
            try {
                // Read the full file
                const fullContent = (0, fileUtils_1.readFileContent)(result.metadata.filePath);
                if (!fullContent) {
                    return result;
                }
