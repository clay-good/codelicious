...
const codeChunker_1 = require("../../embedding/codeChunker");
const semanticChunker_1 = require("../../embedding/semanticChunker");
...
describe.skip('RAG/Embedding/Chunking Stress Tests', () => {
    ...
    beforeEach(() => {
        ...
        const statusBar = {};
        const indexingEngine = new indexer_1.IndexingEngine(mockContext, configManager, statusBar);
        const embeddingManager = new embeddingManager_1.EmbeddingManager(configManager, {});
        const vectorStore = new vectorStore_1.VectorStore(configManager);
        ragService = new ragService_1.RAGService(mockContext, configManager, embeddingManager, vectorStore, indexingEngine);
        embeddingService = new embeddingService_1.EmbeddingService(mockContext, configManager, indexingEngine);
        codeChunker = new codeChunker_1.CodeChunker(512, 50, true);
        semanticChunker = new semanticChunker_1.SemanticChunker(512, 50, 3);
    });
 ...
 `;
    ...
    describe('Embedding Generation Efficiency', () => {
        ...
        it('should batch embed multiple chunks efficiently', async () => {
            ...
            const startTime = Date.now();
            const allChunks = await Promise.all(files.map(f => codeChunker.chunkFile(f.path, f.content)));
            const duration = Date.now() - startTime;
...
//# sourceMappingURL=rag.stress.test.js.map