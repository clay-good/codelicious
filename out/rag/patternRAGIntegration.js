...
exports.PatternRAGIntegration = void 0;
const logger_1 = require("../utils/logger");
const logger = (0, logger_1.createLogger)('PatternRAGIntegration');
class PatternRAGIntegration {
    constructor(ragService, patternLearner, patternRecognizer, patternEmbedding, patternCache, vectorStore, embeddingManager) {
    ...
    */
    async queryWithPatterns(query, options = {}) {
        ...
        const language = options.filters?.language || 'typescript';
        const taskType = options.queryType || 'general';
        const patternRecommendations = await this.getPatternRecommendations(query, language, taskType);
        // Step 4: Enhance results with patterns
        const enhancedResults = await this.enhanceResultsWithPatterns(baseResults.results, patterns, options);
        // Step 5: Re-assemble context with patterns
        const enhancedContext = this.assembleContextWithPatterns(query, enhancedResults, patterns, patternRecommendations, options);
    ...
    */
    async indexPatterns(patterns) {
        ...
        // Add to vector store
        await this.vectorStore.addEmbeddings(vectorStoreEntries);
    ...
    */
    async findRelevantPatterns(query, options) {
        ...
        const qualifiedPatterns = allPatterns.filter((p) => {
            if ('quality' in p && p.quality < minQuality)
                return false;
            if ('successRate' in p && p.successRate < minSuccessRate)
                return false;
            if (options.patternLanguages && !options.patternLanguages.includes(p.language))
                return false;
            return true;
        });
        // Find similar patterns using embeddings
        const similarPatterns = await this.patternEmbedding.findSimilarPatterns(query, qualifiedPatterns, {
            limit: maxPatterns,
            minSimilarity: 0.7,
    ...
    */
    assembleContextWithPatterns(query, results, patterns, recommendations, options) {
    ...
    */
    calculatePatternRelevance(patterns, query) {
        if (patterns.length === 0)
            return 0;
...
exports.PatternRAGIntegration = PatternRAGIntegration;
//# sourceMappingURL=patternRAGIntegration.js.map