...
jest.mock('../../utils/fileUtils', () => ({
 readFileContent: jest.fn((filePath: string) => {
 if (filePath.includes('error')) return null;
 return `// Mock content for ${path.basename(filePath)}`;
 }),
 getFileStats: jest.fn((filePath: string) => ({
 size: 1000,
 modified: Date.now(),
 hash: 'mock-hash'
 })),
...
describe('ParallelIndexer', () => {
 ...
 const indexerWithLowMemory = new ParallelIndexer({
 maxWorkers: 2,
 batchSize: 5,
 memoryLimitMB: 100,
 ...
 const indexer1 = new ParallelIndexer({
 maxWorkers: 2,
 batchSize: 10,
 enableWorkerThreads: false
 });

 const indexer2 = new ParallelIndexer({
 maxWorkers: 2,
 batchSize: 10,
 enableWorkerThreads: false
 });
