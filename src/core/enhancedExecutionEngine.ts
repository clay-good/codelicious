...
const logger = createLogger('EnhancedExecutionEngine');
...
export interface CommandTemplate {
 ...
 description: string;
 command: string;
 variables: string[]; // e.g., ['projectName', 'version']
...
export class EnhancedExecutionEngine extends ExecutionEngine {
 private executionQueue: QueuedExecution[] = [];
 private runningExecutions: Map<string, QueuedExecution> = new Map();
 ...
 */
 protected parseCommandInternal(command: string, useShell: boolean): { cmd: string; args: string[] } {
 if (useShell) {
 ...
 */
 protected prepareEnvironmentInternal(
 ...
 */
 ...
 pid: number,
 callback: (resources: ResourceUsage) => void
 ): NodeJS.Timeout {
 ...
 */
 private stopResourceMonitoring(interval: NodeJS.Timeout): void {
 ...
 */
 private getProcessResources(pid: number): ResourceUsage | null {
 try {
 ...
 */
 private updateResourceStats(resources: ResourceUsage): void {
 ...
 */
 private isRetryableError(error: string, options: RetryOptions): boolean {
 ...
 */
 private async executeQueued(execution: QueuedExecution): Promise<void> {
 try {
 ...
 */
 ...
 command: string,
 profileName: string,
 additionalOptions: StreamingOptions = {}
 ): Promise<ExecutionResult> {
 ...
 */
 private updateMetrics(command: string, result: ExecutionResult): void {
 ...
 */
 getTopCommands(limit: number = 10): CommandStats[] {
 ...
 */
 getSlowestCommands(limit: number = 10): CommandStats[] {
 return Array.from(this.metrics.commandStats.values())
 ...
 */
 getHighestFailureRateCommands(limit: number = 10): CommandStats[] {
 return Array.from(this.metrics.commandStats.values())
 .map(stats => ({
 ...stats,
