...
import { ASTSymbol, ASTAnalysisResult, SymbolRelationship } from './astAnalyzer';

export interface FileNode {
 path: string;
 imports: string[]; // Files this file imports
 exports: string[]; // Symbols this file exports
 dependencies: string[]; // Direct dependencies (files)
 dependents: string[]; // Files that depend on this file
 symbols: ASTSymbol[]; // Symbols defined in this file
...
export interface DependencyGraph {
 ...
 clusters: FileCluster[]; // Groups of related files
}
...
export interface FileCluster {
 ...
 cohesion: number; // How related files are (0-1)
}

export class CrossFileTracker {
 private graph: DependencyGraph;
 private analysisCache: Map<string, ASTAnalysisResult>;
 ...
 */
 private updateRelationships(filePath: string, analysis: ASTAnalysisResult): void {
 const node = this.graph.nodes.get(filePath);
 if (!node) return;
 ...
 */
 ...
 if (
 node1.dependencies.includes(files[j]) ||
 ...
 */
 private inferClusterType(files: string[]): 'module' | 'feature' | 'layer' | 'package' {
