...
export interface ChunkQualityMetrics {
 coherence: number; // 0-1: How semantically coherent
 completeness: number; // 0-1: Contains complete logical units
 size: number; // Optimal size score
 overlap: number; // Overlap quality score
 uniqueness: number; // 0-1: How unique vs duplicated
...
export class ChunkingOptimizer {
 private config: ChunkingOptimizationConfig;
 private chunkHashes: Set<string>;
 private qualityStats: Map<string, ChunkQualityMetrics>;
 ...
 * Score chunk quality
 */
 private scoreChunkQuality(chunks: CodeChunk[]): CodeChunk[] {
 ...
 * Calculate coherence
 */
 private calculateCoherence(chunk: CodeChunk): number {
 ...
 */
 private isAbruptTransition(prev: string, curr: string): boolean {
 ...
 */
 private calculateCompleteness(chunk: CodeChunk): number {
 ...
 */
 private calculateSizeScore(chunk: CodeChunk): number {
 ...
 */
 private calculateOverlapScore(chunk: CodeChunk): number {
 ...
 */
 private calculateUniqueness(chunk: CodeChunk): number {
 ...
 */
 private hashChunk(chunk: CodeChunk): string {
 ...
 * Optimize overlap
 */
 private optimizeOverlap(chunks: CodeChunk[]): CodeChunk[] {
 ...
 */
 private filterLowQuality(chunks: CodeChunk[]): CodeChunk[] {
 ...
 */
 ...
 const lowQualityCount = qualities.filter(m =>
 m.overall < this.config.qualityThreshold
 ).length;

 const duplicateCount = qualities.filter(m =>
 m.uniqueness === 0
 ).length;
