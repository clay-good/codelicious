...
export class VectorStore {
 ...
 collectionName = 'codelicious_embeddings'
 ) {
 ...
 * Update an embedding
 */
 async updateEmbedding(embedding: Embedding): Promise<void> {
 if (!this.isInitialized || !this.collection) {
 throw new Error('VectorStore not initialized');
 }
 ...
 */
 private deserializeMetadata(data: Record<string, any>): EmbeddingMetadata {
 return {
 text: data.text || '',
 source: data.source || '',
