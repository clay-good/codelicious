...
export interface SemanticIntent {
 type: 'data_processing' | 'api_endpoint' | 'validation' | 'transformation' |
 'authentication' | 'authorization' | 'database_operation' | 'file_operation' |
 'network_operation' | 'ui_component' | 'business_logic' | 'utility' | 'test' | 'unknown';
 confidence: number;
 description: string;
 keywords: string[];
}
...
export interface SemanticSimilarity {
 ...
 sharedConcepts: string[]; // Common concepts
}

export interface CodePurpose {
 primary: string; // Main purpose
 secondary: string[]; // Additional purposes
 domain: string; // Domain (e.g., 'web', 'data', 'ml', 'system')
 layer: string; // Architecture layer (e.g., 'controller', 'service', 'model')
}
...
export interface SemanticAnalysis {
 ...
 purpose: CodePurpose;
 complexity: {
 cognitive: number; // How hard to understand (0-100)
 cyclomatic: number; // Cyclomatic complexity
 maintainability: number; // 0-100
 };
 concepts: string[]; // Key concepts in the code
 ...
 */
 async analyze(code: string, filePath: string, astAnalysis?: ASTAnalysisResult): Promise<SemanticAnalysis> {
 ...
 */
 private detectIntent(code: string, filePath: string): SemanticIntent {
 const intents: Array<{ type: SemanticIntent['type']; confidence: number; keywords: string[] }> = [];
 ...
 if (matches > 0) {
 intents.push({
 type: intentType as SemanticIntent['type'],
 ...
 */
 private detectPatterns(code: string, astAnalysis?: ASTAnalysisResult): CodePattern[] {
 ...
 // Strategy pattern
 if (astAnalysis && astAnalysis.symbols.some(s => s.kind === SymbolKind.INTERFACE && /Strategy|Algorithm/.test(s.name))) {
 patterns.push({
 name: 'Strategy',
 type: 'design_pattern',
 confidence: 0.85,
 ...
 if (astAnalysis) {
 const classes = astAnalysis.symbols.filter(s => s.kind === SymbolKind.CLASS);
 for (const cls of classes) {
 const methods = astAnalysis.symbols.filter(s => s.parent === cls.name && s.kind === SymbolKind.METHOD);
 ...
 */
 private classifyPurpose(code: string, filePath: string, intent: SemanticIntent): CodePurpose {
 // Determine domain
 let domain = 'general';
 ...
 * Analyze complexity
 */
 private analyzeComplexity(code: string, astAnalysis?: ASTAnalysisResult): SemanticAnalysis['complexity'] {
 ...
 */
 private extractConcepts(code: string, astAnalysis?: ASTAnalysisResult): string[] {
 ...
 */
 private extractSemanticDependencies(code: string, astAnalysis?: ASTAnalysisResult): string[] {
 ...
 * Assess code quality
 */
 private assessQuality(code: string, patterns: CodePattern[], complexity: SemanticAnalysis['complexity']): SemanticAnalysis['quality'] {
 const issues: string[] = [];
 ...
 */
 private calculateFunctionLengths(code: string): number[] {
 ...
 */
 private getIntentDescription(type: SemanticIntent['type']): string {
 const descriptions: Record<SemanticIntent['type'], string> = {
 ...
 */
 private initializeDesignPatterns(): Map<string, RegExp[]> {
 return new Map([
