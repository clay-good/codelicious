...
describe('VectorStore', () => {
 ...
 vectorStore = new VectorStore(mockConfigManager, 'test_collection');
 });
 ...
 const store = new VectorStore(mockConfigManager);
 expect(store).toBeDefined();
 });

 it('should create vector store with custom collection name', () => {
 const store = new VectorStore(mockConfigManager, 'custom_collection');
 expect(store).toBeDefined();
 });
 });
 ...
 const uninitializedStore = new VectorStore(mockConfigManager);

 await expect(
 uninitializedStore.addEmbeddings([{
 id: 'test',
 vector: [0.1],
 ...
 await expect(
 uninitializedStore.search([0.1, 0.2])
 ).rejects.toThrow('VectorStore not initialized');
 });
 });
 ...
 await expect(
 uninitializedStore.deleteEmbeddings(['test-1'])
 ).rejects.toThrow('VectorStore not initialized');
 });
 });
 ...
 await expect(
 vectorStore.deleteByFilter({ source: 'test.ts' })
