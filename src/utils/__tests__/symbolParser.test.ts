...
class AnotherClass {}
 ...
 const symbols = parseTypeScriptSymbols(code, 'test.ts');
...
interface AnotherInterface {}
 ...
 const symbols = parseTypeScriptSymbols(code, 'test.ts');
 const interfaceSymbols = symbols.filter(s => s.kind === SymbolKind.INTERFACE);
...
const arrowWithParams = (a: string, b: number) => {};
 ...
 const symbols = parseTypeScriptSymbols(code, 'test.ts');
 const arrowSymbols = symbols.filter(s => s.kind === SymbolKind.FUNCTION);
...
enum AnotherEnum {}
 ...
 const symbols = parseTypeScriptSymbols(code, 'test.ts');
 const enumSymbols = symbols.filter(s => s.kind === SymbolKind.ENUM);
...
import defaultExport from 'module3';
 ...
 const imports = parseTypeScriptImports(code);
...
export interface MyInterface {}
 ...
 const exports = parseTypeScriptExports(code);
...
class AnotherClass(BaseClass):
 ...
 const symbols = parsePythonSymbols(code, 'test.py');
 const classSymbols = symbols.filter(s => s.kind === SymbolKind.CLASS);
...
from .local import something
 ...
 const tsSymbols = parseSymbols(tsCode, 'typescript', 'test.ts');
 const pySymbols = parseSymbols(pyCode, 'python', 'test.py');
 ...
 const symbols = parseSymbols('some code', 'unknown', 'test.txt');
 ...
 const tsImports = parseImports(tsCode, 'typescript');
 const pyImports = parseImports(pyCode, 'python');
 ...
 const tsExports = parseExports(tsCode, 'typescript');
