...
import { PriorityQueue, calculateFilePriority } from '../priorityQueue';

describe('PriorityQueue', () => {
 let queue: PriorityQueue<string>;

 beforeEach(() => {
 queue = new PriorityQueue<string>(item => item);
 });
...
describe('calculateFilePriority', () => {
 ...
 const openPriority = calculateFilePriority('file.ts', 1000, Date.now(), true, false);
 const closedPriority = calculateFilePriority('file.ts', 1000, Date.now(), false, false);

 expect(openPriority).toBeGreaterThan(closedPriority);
 });

 it('should give high priority to recently modified files', () => {
 const recentPriority = calculateFilePriority('file.ts', 1000, Date.now(), false, true);
 const oldPriority = calculateFilePriority('file.ts', 1000, Date.now(), false, false);

 expect(recentPriority).toBeGreaterThan(oldPriority);
 });

 it('should give higher priority to smaller files', () => {
 const smallPriority = calculateFilePriority('file.ts', 5000, Date.now());
 const largePriority = calculateFilePriority('file.ts', 200000, Date.now());

 expect(smallPriority).toBeGreaterThan(largePriority);
 });

 it('should give high priority to index files', () => {
 const indexPriority = calculateFilePriority('index.ts', 10000, Date.now());
 const regularPriority = calculateFilePriority('regular.ts', 10000, Date.now());

 expect(indexPriority).toBeGreaterThan(regularPriority);
 });

 it('should give high priority to main files', () => {
 const mainPriority = calculateFilePriority('main.ts', 10000, Date.now());
 const regularPriority = calculateFilePriority('regular.ts', 10000, Date.now());
 ...
 const packagePriority = calculateFilePriority('package.json', 10000, Date.now());
 const regularPriority = calculateFilePriority('regular.ts', 10000, Date.now());
 ...
 const testPriority = calculateFilePriority('file.test.ts', 10000, Date.now());
 const regularPriority = calculateFilePriority('file.ts', 10000, Date.now());

 expect(testPriority).toBeLessThan(regularPriority);
 });

 it('should never return negative priority', () => {
 const priority = calculateFilePriority('test.spec.ts', 1000000, Date.now());
 expect(priority).toBeGreaterThanOrEqual(0);
 });

 it('should combine multiple factors correctly', () => {
 const highPriority = calculateFilePriority(
 'index.ts',
 5000,
 Date.now(),
 true,
 ...
 const lowPriority = calculateFilePriority(
 'file.test.ts',
 200000,
 Date.now(),
 false,
