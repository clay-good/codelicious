...
describe('ComplexityAnalyzer', () => {
 ...
 expect(calculateCyclomaticComplexity(code)).toBeGreaterThanOrEqual(5);
 });
 ...
 `;
 // 1 base + 1 if + 1 && + 1 || = 4
 expect(calculateCyclomaticComplexity(code)).toBeGreaterThanOrEqual(3);
 });
 ...
 `;
 expect(calculateCyclomaticComplexity(code)).toBeGreaterThanOrEqual(4); // 1 base + 3 cases
 });
 ...
 `;
 expect(calculateCyclomaticComplexity(code)).toBe(2); // 1 base + 1 catch
 });
 });
 ...
 console.log(x);
 `;
 expect(calculateCognitiveComplexity(code)).toBe(0);
 });
 ...
 // Total: 3
 expect(calculateCognitiveComplexity(code)).toBeGreaterThanOrEqual(3);
 });
 ...
 `;
 expect(calculateCognitiveComplexity(code)).toBeGreaterThanOrEqual(4);
 });
 });
 ...
 console.log(x);
 `;
 expect(calculateCommentRatio(code)).toBe(0);
 });
 ...
 console.log(x);
 `;
 const ratio = calculateCommentRatio(code);
 ...
 */
 ...
 const issues = detectComplexityIssues(metrics);
 ...
 expect(getComplexityLevel(3, 'cyclomatic')).toBe('low');
 expect(getComplexityLevel(8, 'cyclomatic')).toBe('medium');
 expect(getComplexityLevel(13, 'cyclomatic')).toBe('high');
 expect(getComplexityLevel(25, 'cyclomatic')).toBe('critical');
 });

 it('should return correct level for cognitive complexity', () => {
 expect(getComplexityLevel(3, 'cognitive')).toBe('low');
 expect(getComplexityLevel(8, 'cognitive')).toBe('medium');
 expect(getComplexityLevel(18, 'cognitive')).toBe('critical'); // 18 > 15 (high threshold)
 expect(getComplexityLevel(25, 'cognitive')).toBe('critical');
 });

 it('should return correct level for nesting', () => {
 expect(getComplexityLevel(1, 'nesting')).toBe('low');
 expect(getComplexityLevel(2, 'nesting')).toBe('low');
 expect(getComplexityLevel(4, 'nesting')).toBe('high');
 expect(getComplexityLevel(7, 'nesting')).toBe('critical');
 });
 });
});
