...
describe.skip('DistributedProcessor', () => {
 let processor: DistributedProcessor;
 ...
 const createTestFile = (name: string, content: string = ''): GeneratedCode => ({
 filePath: name,
 ...
 const files: GeneratedCode[] = Array.from({ length: 100 }, (_, i) =>
 ...
 const files: GeneratedCode[] = Array.from({ length: 50 }, (_, i) =>
 createTestFile(`test${i}.ts`)
 );
 ...
 // First run - no cache
 const result1 = await processor.processFiles(
 files,
 'process-file',
 (file) => ({ processed: true })
 );
 ...
 const result2 = await processor.processFiles(
 files,
 'process-file',
 (file) => ({ processed: true })
 );
 ...
 const mockProcessor = (file: GeneratedCode) => {
 ...
 const files: GeneratedCode[] = Array.from({ length: 20 }, (_, i) =>
 createTestFile(`test${i}.ts`)
 );
 ...
 // Test with 2 workers
 const processor2 = new DistributedProcessor({
 ...
 // Test with 4 workers
 const processor4 = new DistributedProcessor({
