 ...
 */
 private formatMarkdown(result: RetrievalResult, includeMetadata: boolean): string {
 const parts: string[] = [];
 ...
 */
 private formatXML(result: RetrievalResult, includeMetadata: boolean): string {
 const parts: string[] = ['<code_snippet>'];
 ...
 */
 private formatPlain(result: RetrievalResult, includeMetadata: boolean): string {
 const parts: string[] = [];
 ...
 */
 private formatContext(sections: string[], format: 'markdown' | 'xml' | 'plain'): string {
 if (format === 'xml') {
 ...
 * Build system prompt
 */
 private buildSystemPrompt(resultCount: number, truncated: boolean): string {
 const parts: string[] = [
 ...
 */
 private buildUserMessage(query: string, context: string, format: 'markdown' | 'xml' | 'plain'): string {
 if (format === 'xml') {
 return `<user_query>\n${query}\n</user_query>\n\n${context}`;
 } else {
 return `${context}\n\n## User Query\n\n${query}`;
 ...
 */
 addSpecializedContext(
 context: AssembledContext,
 queryType: 'error' | 'test' | 'documentation' | 'refactor'
 ): AssembledContext {
