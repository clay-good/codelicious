...
import { RequirementsParser, ParsedRequirements } from './requirementsParser';
import { IntelligentPlanner, ExecutionPlan } from './intelligentPlanner';
import { ContextAwareCodeGenerator, GenerationOptions, GenerationResult } from './contextAwareCodeGenerator';
import { AutomaticTestGenerator, TestGenerationResult } from './automaticTestGenerator';
...
import { CancellationToken, CancellationTokenSource, CancellationError } from '../utils/cancellationToken';
import { MCPToolRegistry, ToolRecommendation } from '../integrations/mcpToolRegistry';
import { IterativeRefinementEngine, RefinementResult } from './iterativeRefinementEngine';
import { BuildErrorFixer } from './buildErrorFixer';
...
export interface WorkflowOptions {
 ...
 enableIterativeRefinement?: boolean; // Enable iterative refinement loop
 enableRealtimeCompilation?: boolean; // Enable real-time compilation
 enableDependencyResolution?: boolean; // Enable automatic dependency resolution
 targetQualityScore?: number; // Target quality score (0-100)
 maxRefinementIterations?: number; // Max refinement iterations
 generateReadme?: boolean; // Generate professional README (default: true, no emojis)
}
...
export interface WorkflowResult {
 ...
 duration: number;
 errors: string[];
 mcpToolsUsed?: Array<{ toolId: string; operation: string; success: boolean }>; // Track MCP tool usage
 refinementResult?: RefinementResult; // Iterative refinement results
 dependenciesInstalled?: number; // Number of dependencies installed
 compilationAttempts?: number; // Number of compilation attempts
 qualityScore?: number; // Final quality score
}
...
export class AutonomousWorkflow {
 ...
 private testGenerator: AutomaticTestGenerator;
 private validator: ProductionValidator;
 private readmeGenerator: ProfessionalReadmeGenerator;
 private cancellationSource?: CancellationTokenSource;
 private cleanupCallbacks: Array<() => void> = [];
 private mcpRegistry?: MCPToolRegistry; // MCP tool registry
 private refinementEngine: IterativeRefinementEngine; // NEW: Iterative refinement
 private buildErrorFixer: BuildErrorFixer; // NEW: Build error fixer
 private dependencyResolver: DependencyResolver; // NEW: Dependency resolver
 private realtimeCompiler: RealtimeCompiler; // NEW: Real-time compiler
 ...
 this.requirementsParser = new RequirementsParser(orchestrator);
 this.planner = new IntelligentPlanner(orchestrator);
 this.codeGenerator = new ContextAwareCodeGenerator(orchestrator, workspaceRoot);
 this.testGenerator = new AutomaticTestGenerator(orchestrator, workspaceRoot);
 this.validator = new ProductionValidator(executionEngine, workspaceRoot);
 this.readmeGenerator = new ProfessionalReadmeGenerator(orchestrator, workspaceRoot);
 this.mcpRegistry = mcpRegistry;

 // Initialize new autonomous components
 this.refinementEngine = new IterativeRefinementEngine(orchestrator, this.validator, executionEngine, workspaceRoot);
 this.buildErrorFixer = new BuildErrorFixer(orchestrator, executionEngine, workspaceRoot);
 this.dependencyResolver = new DependencyResolver(executionEngine, orchestrator, workspaceRoot);
 this.realtimeCompiler = new RealtimeCompiler(executionEngine, orchestrator, workspaceRoot);
 }
 ...
 */
 ...
 const compileResult = await this.realtimeCompiler.compileIncremental(
 ...
 const generatedTests = await this.testGenerator.generate(
 compiledCode.generatedFiles,
 context
 );
 ...
 refinementResult = await this.refinementEngine.refineUntilPerfect(
 compiledCode,
 requirements,
 {
 ...
 generatedCode: {} as GenerationResult,
 generatedTests: {} as TestGenerationResult,
 ...
 */
 async recommendMCPTools(task: string): Promise<ToolRecommendation[]> {
 if (!this.mcpRegistry) {
 return [];
 }
