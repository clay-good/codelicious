...
import { ComprehensiveTestGenerator, TestGenerationRequest, TestType } from '../testing/comprehensiveTestGenerator';
import { createLogger } from '../utils/logger';

const logger = createLogger('AutomaticTestGenerator');
...
export class AutomaticTestGenerator {
 ...
 this.comprehensiveGenerator = new ComprehensiveTestGenerator(orchestrator);
 }
 ...
 */
 private async detectTestPattern(context: ArchitecturalContext): Promise<TestPattern> {
 // Look for existing test files in context
 const testFiles = context.relevantFiles.filter(f =>
 ...
 // Detect framework
 let framework: TestPattern['framework'] = 'jest';
 ...
 */
 private getFrameworkImports(framework: TestPattern['framework']): string[] {
 switch (framework) {
 case 'jest':
 ...
 */
 private async generateComprehensiveTestForFile(
 code: GeneratedCode,
 pattern: TestPattern,
 context: ArchitecturalContext
 ...
 */
 ...
 const testTypes: ('unit' | 'integration' | 'e2e')[] = ['unit'];
 ...
 */
 private detectFramework(filePath: string, pattern: TestPattern): string | undefined {
 ...
 if (content.includes('express') || content.includes('route')) return 'express';
 ...
 */
 private isAPICode(content: string): boolean {
 const apiPatterns = [
 /router\./,
 ...
 */
 private isUICode(content: string): boolean {
 const uiPatterns = [
 /React\.FC/,
 /useState/,
 /useEffect/,
 ...
 */
 private formatTestFile(testCode: string, pattern: TestPattern): string {
 ...
 * Get test file path
 */
 private getTestPath(sourcePath: string, pattern: TestPattern): string {
 ...
 */
 private estimateFileCoverage(testContent: string, sourceContent: string): number {
 ...
 */
 private estimateCoverage(tests: GeneratedTest[], generatedCode: GeneratedCode[]): number {
