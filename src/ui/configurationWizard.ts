...
const logger = createLogger('ConfigurationWizard');
 ...
 */
 ...
 ' Welcome to Codelicious! Let\'s get you set up with the optimal configuration for your project.',
 'Get Started',
 'Skip Setup'
 );
 ...
 */
 ...
 if (deps['@angular/core']) analysis.frameworks.push('Angular');
 if (deps['next']) analysis.frameworks.push('Next.js');
 if (deps['express']) analysis.frameworks.push('Express');
 if (deps['nestjs']) analysis.frameworks.push('NestJS');
 ...
 if (reqs.includes('pytest')) analysis.hasTests = true;
 }
 }
 ...
 * Show analysis results
 */
 private async showAnalysisResults(analysis: ProjectAnalysis): Promise<void> {
 const details = [
 ...
 * Configure API keys
 */
 private async configureApiKeys(analysis: ProjectAnalysis): Promise<void> {
 ...
 */
 private getModelRecommendation(analysis: ProjectAnalysis): ConfigurationRecommendation {
 ...
 * Configure model preferences
 */
 private async configureModelPreferences(analysis: ProjectAnalysis): Promise<void> {
 ...
 await config.update('models.costLimit', costLimits[budget.value as keyof typeof costLimits], true);
 }
 }
 ...
 * Configure features
 */
 private async configureFeatures(analysis: ProjectAnalysis): Promise<void> {
 ...
 */
 ...
 await vscode.window.showInformationMessage(
 'Setup Complete!\n\nCodelicious is now configured and ready to use.\n\nTry opening the chat (Cmd+Shift+L) or run a tutorial to learn more!',
 'Start Tutorial',
 'Open Chat',
 'Close'
 ).then(action => {
