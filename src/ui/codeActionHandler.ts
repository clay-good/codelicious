...
const logger = createLogger('CodeActionHandler');
 ...
 */
 async applyCode(code: string, language: string, suggestedFileName?: string): Promise<void> {
 try {
 ...
 */
 private detectFilePathFromCode(code: string, language: string): string | undefined {
 ...
 */
 private suggestFilePath(name: string, language: string): string {
 ...
 */
 private async showDiffPreview(filePath: string, newContent: string): Promise<boolean> {
 try {
 ...
 originalUri,
 modifiedUri,
 `${path.basename(filePath)} (Original ↔ Modified)`
 );
 ...
 */
 private async createNewFile(code: string, language: string, suggestedFileName?: string): Promise<void> {
 ...
 */
 private async insertIntoCurrentFile(code: string): Promise<void> {
 ...
 */
 private async replaceSelection(code: string): Promise<void> {
 const editor = vscode.window.activeTextEditor;
 if (!editor) {
 throw new Error('No active editor');
 }
 ...
 */
 ...
 `Run this ${language} code? This will execute the code in your environment.`,
 { modal: true },
 'Run', 'Cancel'
 );
