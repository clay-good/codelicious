...
describe('TestManager', () => {
 ...
 let mockTestGenerator: jest.Mocked<TestGenerator>;
 let mockTestExecutor: jest.Mocked<TestExecutor>;
 ...
 (vscode.window as any).withProgress = jest.fn().mockImplementation(async (options, task) => {
 ...
 (TestGenerator as jest.Mock).mockImplementation(() => mockTestGenerator);
 ...
 (TestExecutor as jest.Mock).mockImplementation(() => mockTestExecutor);

 testManager = new TestManager(workspaceRoot);
 });
 ...
 expect(mockTestGenerator.generateTestsForFile).toHaveBeenCalledWith(
 ...
 'Cannot generate tests for test files'
 );
 });
 ...
 expect.stringContaining('Failed to generate tests')
 );
 });
 ...
 expect(mockTestExecutor.runTestFile).toHaveBeenCalledWith(
 '/test/workspace/src/utils.test.ts'
 );
 });
 ...
 expect(vscode.window.showWarningMessage).toHaveBeenCalledWith(
 'No test file found for current file'
 );
 });
 });
