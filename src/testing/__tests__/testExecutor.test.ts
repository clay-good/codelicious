...
describe('TestExecutor', () => {
 ...
 testExecutor = new TestExecutor(workspaceRoot);
 });
 ...
 'npm',
 expect.arrayContaining(['--testPathPattern', filePath]),
 expect.any(Object)
 );
 });
 });
 ...
 expect(spawn).toHaveBeenCalledWith(
 'npm',
 expect.arrayContaining(['--testPathPattern', 'utils']),
 expect.any(Object)
 );
 });
 ...
 expect(spawn).toHaveBeenCalledWith(
 'npm',
 expect.arrayContaining(['--maxWorkers', '4']),
 expect.any(Object)
 );
 });
 });
