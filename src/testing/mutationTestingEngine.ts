...
const logger = createLogger('MutationTestingEngine');

export interface MutationTestResult {
 mutationScore: number; // 0-100 (% of mutations killed)
...
export type MutationType =
 | 'arithmetic-operator' // + to -, * to /, etc.
 | 'comparison-operator' // > to <, === to !==, etc.
 | 'logical-operator' // && to ||, ! to identity
 | 'conditional-boundary' // < to <=, > to >=
 | 'return-value' // return true to return false
 | 'remove-condition' // if (x) to if (true)
 | 'negate-condition' // if (x) to if (!x)
 | 'remove-statement' // delete line
 | 'constant-replacement'; // 0 to 1, '' to 'x'
 ...
 */
 async test(sourceFile: string, testFile: string, code: string): Promise<MutationTestResult> {
 ...
 let killedCount = 0;
 const survivedMutations: Mutation[] = [];
 ...
 */
 private generateMutations(code: string, filePath: string): Mutation[] {
 ...
 */
 private async identifyWeakTests(
 survivedMutations: Mutation[],
 testFile: string
 ): Promise<WeakTest[]> {
 ...
 const mutationsByLine = new Map<number, Mutation[]>();
 ...
 */
 private generateTestSuggestion(mutations: Mutation[]): string {
 ...
 */
 ...
 // Helper methods
 private getMutatedArithmeticOperators(operator: ts.SyntaxKind): string[] {
 ...
 private getMutatedComparisonOperators(operator: ts.SyntaxKind): string[] {
 const map: Record<number, string[]> = {
