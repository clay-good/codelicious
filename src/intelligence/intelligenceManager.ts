...
import { CodeAnalyzer, AnalysisResult } from './codeAnalyzer';
...
export class IntelligenceManager {
 ...
 this.codeAnalyzer = new CodeAnalyzer(workspaceRoot);
 ...
 */
 ...
 ` Analyzed ${files.length} files successfully!`
 );
 });
 } catch (error) {
 vscode.window.showErrorMessage(
 ...
 */
 async extractMethod(): Promise<void> {
 const editor = vscode.window.activeTextEditor;
 if (!editor) {
 vscode.window.showWarningMessage('No active editor');
 return;
 ...
 try {
 const result = await this.refactoringEngine.extractMethod(
 editor.document,
 selection,
 methodName
 );
 ...
 if (apply === 'Apply') {
 await this.refactoringEngine.applyRefactoring(
 editor.document,
 result.operations
 );
 ...
 */
 async extractVariable(): Promise<void> {
 const editor = vscode.window.activeTextEditor;
 if (!editor) {
 vscode.window.showWarningMessage('No active editor');
 return;
 ...
 */
 async showCodeQualityReport(): Promise<void> {
 ...
 */
 private showDependencyResults(graph: DependencyGraph): void {
 ...
 * Format top issues
 */
 private formatTopIssues(results: AnalysisResult[]): string {
 ...
 */
 private formatRecommendations(results: AnalysisResult[]): string {
