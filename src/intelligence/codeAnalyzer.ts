 ...
 */
 ...
 const mi = Math.max(
 0,
 100 - (complexity * 2) - (linesOfCode / 10) + (commentRatio * 20)
 );
 ...
 * Detect code issues
 */
 private detectIssues(content: string, metrics: CodeMetrics): CodeIssue[] {
 ...
 * Detect code smells
 */
 private detectCodeSmells(content: string): CodeIssue[] {
 const issues: CodeIssue[] = [];
 const lines = content.split('\n');
 ...
 */
 private calculateScore(metrics: CodeMetrics, issues: CodeIssue[]): number {
 let score = 100;
