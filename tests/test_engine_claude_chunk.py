...
class TestClaudeRunBuildCycle:
    """Coverage for run_build_cycle legacy interface (lines 273-293)."""

    def test_run_build_cycle_no_specs(self, tmp_path: pathlib.Path) -> None:
        """Lines 280-282: discover_incomplete_specs returns [] → immediate success."""
        engine = ClaudeCodeEngine()
        ...
        with mock.patch("codelicious.spec_discovery.discover_incomplete_specs", return_value=[]):
            result = engine.run_build_cycle(tmp_path, git_manager, cache_manager)

        assert result.success is True
        assert "no incomplete" in result.message.lower()

    def test_run_build_cycle_with_specs(self, tmp_path: pathlib.Path) -> None:
        """Lines 284-293: discover_incomplete_specs returns specs, delegates to V2Orchestrator.run."""
        engine = ClaudeCodeEngine()
        git_manager = mock.MagicMock()
        cache_manager = mock.MagicMock()

        fake_spec = tmp_path / "docs" / "specs" / "01_feature.md"
        fake_spec.parent.mkdir(parents=True)
        fake_spec.write_text("# Spec\n- [ ] task\n")

        from codelicious.orchestrator import OrchestratorResult

        orch_result = OrchestratorResult(success=True, message="all done", elapsed_s=1.5)
        ...
        with mock.patch("codelicious.spec_discovery.discover_incomplete_specs", return_value=[fake_spec]):
            with mock.patch("codelicious.orchestrator.V2Orchestrator", return_value=mock_orch) as mock_orch_cls:
                result = engine.run_build_cycle(tmp_path, git_manager, cache_manager, push_pr=True)

        assert result.success is True
        assert result.message == "all done"
        mock_orch_cls.assert_called_once()
        mock_orch.run.assert_called_once()
        # push_pr kwarg should have been forwarded
        call_kwargs = mock_orch.run.call_args.kwargs
        assert call_kwargs.get("push_pr") is True
